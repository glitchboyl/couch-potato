...
SOUL: `references/souls/tester.md`
...
8. **Results**: Write two files to `.couch/requirements/<req-id>/test-reports/`:
   - `<task-id>.json` — machine-readable status per `verification.json` schema in `references/schemas.md`. Team Lead reads this for wave decisions.
