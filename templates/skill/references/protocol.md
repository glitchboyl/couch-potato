...
5. Detect active dev server — read ports from `.couch/config.json` `server_ports`
6. Note `frontend_path` from config — include in spawn prompts for agents working within the frontend module
7. `TeamCreate` with `team_name: <req-id>` — this creates the shared task list and messaging channel for the swarm
...
For temporary discussion agents: include the Challenger SOUL (`references/souls/challenger.md`) + codex SKILL instructions. These are one-shot — they participate in a specific discussion and are not permanent team members.
