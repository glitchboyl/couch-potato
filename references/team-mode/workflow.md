# Couch Potato — Workflow Reference (Team Mode)
...
**Plan** — Requirement confirmed → give to Architect for task breakdown. Architect produces `requirement.md` (canonical spec) and `tasks.json` (task plan). Confirm which agents are needed per task. Exit: both `requirement.md` and `tasks.json` received; `tasks.json` validated per `${CLAUDE_PLUGIN_ROOT}/references/schemas.md` validation rules.
