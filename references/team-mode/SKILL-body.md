...
**An agent is blocked by a hook (path restriction, tool restriction, policy).** The agent will return the exact edit it would have made along with the hook error. You MUST NOT apply that edit using your own tools (`Bash`, `Edit`, `Write`) — see `## You don't write code`. Applying the edit directly invalidates downstream verification gates (Architect review, Wave Exit Checklist, Tester). The correct response: escalate to the user with (a) the agent's proposed edit verbatim, (b) the hook that blocked it, and (c) the options — remove the hook registration and let the agent retry, or apply the edit manually outside the swarm and mark the task out-of-scope. The grounding incident is the global `PreToolUse` hook bug in `hooks/hooks.json` that blocked req-010 execution (now fixed by req-011 task-002). See `.couch/retrospectives/req-010.md`. If the requirement itself requires changing the hook registration, see the authorized bootstrap prerequisite bullet below.

**A requirement targets the hook system itself (authorized bootstrap prerequisite).** This is the narrow carve-out to the "you may not apply the fix yourself" rule (Proposal B) for the chicken-and-egg case where the requirement's target files are blocked by the very hooks it needs to change — making normal team execution impossible. Four constraints bound its use: (i) applies ONLY to infrastructure/bootstrap work that is a prerequisite for the team to function at all — not to agent-escalated mid-execution blocks (that is the hook-blocked situation above); (ii) requires EXPLICIT user authorization — a clear affirmative ("yes, do it") is required; absence of objection or implicit consent does not qualify; (iii) must be followed by a Coder verification task that confirms the bootstrap took effect, making the episode auditable rather than a hand-wave; (iv) Team Lead must ensure the episode is named in the run's retrospective as a special case, with what was authorized, what was done, and what verification passed captured — the format is Retrospective Agent's judgment, not Team Lead's to prescribe. For mid-Execute hook blocks that do not require changing the registration itself, see the hook-blocked bullet above.
